/* ================================================================
 *  SDL Testimonial Slider v1.0
 *  A no-constructor IIFE that reads window.SDLTestimonialSliderConfig,
 *  detects Summary Blocks inside sections with matching IDs, extracts
 *  testimonial data, and builds a Swiper-powered testimonial slider.
 *
 *  ACTIVATION: Set a Squarespace Section's ID to:
 *    "testimonial-slider"  or  "testimonial-slider-1"  etc.
 *  (c) Square Design Lab
 * ================================================================ */
(function () {
  'use strict';
  ...
  /* ==================================================================
   *  BUNDLED SWIPER 11.2.1  (minified, all required modules)
   * ================================================================== */
          ...
          i >= 0 && (g[i] = e),
            g.length < 2 ||
              ...
              v.imageEl &&
                ...
                a.scale > v.maxRatio &&
                  (a.scale =
                    v.maxRatio - 1 + (a.scale - v.maxRatio + 1) ** 0.5),
                a.scale < s.minRatio &&
                  (a.scale =
   ...
   *  CONSTANTS
   * ────────────────────────────────────────────────────────────── */
  var NAMESPACE = 'SDLTestimonialSlider';
  var SECTION_ID_RE = /^testimonial-slider(-\d+)?$/;
  var CSS_PREFIX = 'sdl-ts-';
  ...
  var DEFAULTS = {
    ...
    // Content mapping
    nameField: 'title',
    roleField: 'categories',  // 'categories' | 'tags' | 'metadata-secondary'
    quoteField: 'excerpt',
    ...
    // Review layout
    reviewCardBg: '#1a3a2a',
    reviewCardColor: '#f5f0e8',
    reviewStarColor: '#f5a623',

    // Tilted layout
    tiltColors: ['#2d4a9e', '#e85d3a', '#1a7a5a', '#e85d3a', '#2d4a9e', '#1a7a5a'],
    tiltAngle: 3,
  };
   ...
   *  FOUC PREVENTION
   * ────────────────────────────────────────────────────────────── */
  var foucStyleEl = null;

  function injectFOUCStyles() {
    if (foucStyleEl) return;
    foucStyleEl = document.createElement('style');
    foucStyleEl.id = CSS_PREFIX + 'fouc';
    foucStyleEl.textContent =
      'section[id^="testimonial-slider"] .summary-block-wrapper { opacity: 0; transition: opacity 0.3s; }';
    document.head.appendChild(foucStyleEl);
  }
  ...
  function isEditMode() {
    return !!(
      ...
      (window.self !== window.top && document.querySelector('[data-squarespace-management-id]'))
    );
  }
  ...
  function getUserConfig() {
    return window.SDLTestimonialSliderConfig || {};
  }

  function getSettings(sectionEl) {
    var userConfig = getUserConfig();
    var globalDefaults = userConfig.defaults || {};
    var sectionId = sectionEl.id || '';
    var sectionOverrides = (userConfig.sections && userConfig.sections[sectionId])
      || (userConfig[sectionId] && typeof userConfig[sectionId] === 'object' ? userConfig[sectionId] : {});
    var merged = deepMerge({}, DEFAULTS, globalDefaults, sectionOverrides);

    // Layout-specific slidesPerView overrides if user didn't explicitly set it
    if (!sectionOverrides.slidesPerView && !globalDefaults.slidesPerView) {
  ...
  function findSections() {
    ...
    all.forEach(function (sec) {
      if (SECTION_ID_RE.test(sec.id)) matched.push(sec);
    });
    return matched;
  }

  /* ──────────────────────────────────────────────────────────────────
   *  EXTRACT TESTIMONIAL DATA FROM SUMMARY ITEMS
   * ────────────────────────────────────────────────────────────── */
  function extractTestimonials(sectionEl, settings) {
    var summaryBlock = sectionEl.querySelector('.sqs-block-summary-v2');
    if (!summaryBlock) return [];

    var items = summaryBlock.querySelectorAll('.summary-item');
    var testimonials = [];
    ...
    items.forEach(function (item) {
      ...
      // Name — from title
      var titleEl = item.querySelector('.summary-title a.summary-title-link, .summary-title');
      data.name = titleEl ? titleEl.textContent.trim() : '';

      // Quote — from excerpt
      var excerptEl = item.querySelector('.summary-excerpt p, .summary-excerpt');
      data.quote = excerptEl ? excerptEl.textContent.trim() : '';

      // Role — from configured field
      if (settings.roleField === 'categories' || settings.roleField === 'metadata-secondary') {
        var catEl = item.querySelector('.summary-metadata--primary');
        if (catEl) {
          var catLinks = catEl.querySelectorAll('a');
          if (catLinks.length) {
            var parts = [];
            catLinks.forEach(function (a) { parts.push(a.textContent.trim()); });
            data.role = parts.join(', ');
          } else {
            data.role = catEl.textContent.trim();
          }
        }
      }
      if (settings.roleField === 'tags') {
        var tagEl = item.querySelector('.summary-metadata--secondary');
        if (tagEl) {
          var tagLinks = tagEl.querySelectorAll('a');
          if (tagLinks.length) {
            var parts2 = [];
            tagLinks.forEach(function (a) { parts2.push(a.textContent.trim()); });
            data.role = parts2.join(', ');
          } else {
            data.role = tagEl.textContent.trim();
          }
        }
      }
      ...
      // Thumbnail
      var imgEl = item.querySelector('.summary-thumbnail-image, .summary-thumbnail img');
      if (imgEl) {
        data.image = imgEl.getAttribute('data-src') || imgEl.getAttribute('src') || '';
        data.imageAlt = imgEl.getAttribute('alt') || data.name;
      } else {
        data.image = '';
        data.imageAlt = '';
      }
  ...
  /* ──────────────────────────────────────────────────────────────────
   *  STYLE INHERITANCE — read computed site styles
   * ────────────────────────────────────────────────────────────── */
  function readSiteStyles(sectionEl) {
    var styles = {
      fontFamily: '',
      titleColor: '',
      bodyColor: '',
    ...
    var titleEl = sectionEl.querySelector('.summary-title a, .summary-title, h2, h3');
    var bodyEl = sectionEl.querySelector('.summary-excerpt p, .summary-excerpt, p');
    if (titleEl) {
      var cs = window.getComputedStyle(titleEl);
      styles.fontFamily = cs.fontFamily;
      styles.titleColor = cs.color;
    }
    if (bodyEl) {
      var cs2 = window.getComputedStyle(bodyEl);
      styles.bodyColor = cs2.color;
      if (!styles.fontFamily) styles.fontFamily = cs2.fontFamily;
    }
    var sectionCs = window.getComputedStyle(sectionEl);
    styles.bgColor = sectionCs.backgroundColor;
    return styles;
  }
   ...
   *  DEFAULT ARROW ICONS
   * ────────────────────────────────────────────────────────────── */
  var DEFAULT_PREV_ICON = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>';
  var DEFAULT_NEXT_ICON = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 6 15 12 9 18"></polyline></svg>';

  /* ──────────────────────────────────────────────────────────────────
   *  QUOTE ICON SVG
   * ────────────────────────────────────────────────────────────── */
  var QUOTE_ICON_SVG = '<svg class="' + CSS_PREFIX + 'quote-icon" width="32" height="32" viewBox="0 0 24 24" fill="currentColor" opacity="0.15"><path d="M6 17h3l2-4V7H5v6h3zm8 0h3l2-4V7h-6v6h3z"/></svg>';

  /* ──────────────────────────────────────────────────────────────────
   *  STAR RATING HTML
   * ────────────────────────────────────────────────────────────── */
  function buildStarRating(count) {
    var stars = '';
    for (var i = 0; i < count; i++) {
      stars += '<svg class="' + CSS_PREFIX + 'star" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>';
    }
    return '<div class="' + CSS_PREFIX + 'stars">' + stars + '</div>';
  }

  /* ──────────────────────────────────────────────────────────────────
   *  BUILD SLIDE HTML PER LAYOUT
   * ────────────────────────────────────────────────────────────── */
  function buildSlideHTML(testimonial, index, settings) {
    var layout = settings.layout;
    var html = '';

    if (layout === 'cards') {
      html += '<div class="' + CSS_PREFIX + 'card">';
      ...
      html += '<div class="' + CSS_PREFIX + 'spotlight">';
      html += QUOTE_ICON_SVG.replace('opacity="0.15"', 'opacity="0.12"').replace('width="32" height="32"', 'width="48" height="48"');
      html += '<p class="' + CSS_PREFIX + 'quote">' + testimonial.quote + '</p>';
      html += '<div class="' + CSS_PREFIX + 'author">';
      if (testimonial.image) {
        html += '<img class="' + CSS_PREFIX + 'avatar" src="' + testimonial.image + '" alt="' + testimonial.imageAlt + '" loading="lazy" />';
      ...
      html += '<div class="' + CSS_PREFIX + 'minimal-card">';
      html += '<span class="' + CSS_PREFIX + 'number">' + pad(index + 1) + '</span>';
      html += '<p class="' + CSS_PREFIX + 'quote">' + testimonial.quote + '</p>';
      html += '<div class="' + CSS_PREFIX + 'author-info">';
      html += '<span class="' + CSS_PREFIX + 'name">' + testimonial.name + '</span>';
      if (testimonial.role) {
      ...
      html += '<div class="' + CSS_PREFIX + 'magazine">';
      if (testimonial.image) {
        html += '<div class="' + CSS_PREFIX + 'magazine-image"><img src="' + testimonial.image + '" alt="' + testimonial.imageAlt + '" loading="lazy" /></div>';
      }
      html += '<div class="' + CSS_PREFIX + 'magazine-content">';
      html += QUOTE_ICON_SVG;
      html += '<p class="' + CSS_PREFIX + 'quote">' + testimonial.quote + '</p>';
      html += '<div class="' + CSS_PREFIX + 'author-info">';
      html += '<span class="' + CSS_PREFIX + 'name">' + testimonial.name + '</span>';
      ...
      var palette = settings.colorPalette || DEFAULTS.colorPalette;
      var bgColor = palette[index % palette.length];
      html += '<div class="' + CSS_PREFIX + 'colorful-card" style="background:' + bgColor + ';">';
      html += '<p class="' + CSS_PREFIX + 'quote">&ldquo;' + testimonial.quote + '&rdquo;</p>';
      html += '<div class="' + CSS_PREFIX + 'author-info">';
      html += '<span class="' + CSS_PREFIX + 'name">' + testimonial.name + '</span>';
      if (testimonial.role) {
        html += '<span class="' + CSS_PREFIX + 'role">' + testimonial.role + '</span>';
      ...
      var reviewBg = settings.reviewCardBg || DEFAULTS.reviewCardBg;
      var reviewColor = settings.reviewCardColor || DEFAULTS.reviewCardColor;
      html += '<div class="' + CSS_PREFIX + 'review-card" style="background:' + reviewBg + '; color:' + reviewColor + ';">';
      html += buildStarRating(settings.starCount);
      if (testimonial.role) {
        html += '<span class="' + CSS_PREFIX + 'review-tag">' + testimonial.role + '</span>';
      }
      html += '<p class="' + CSS_PREFIX + 'quote">' + testimonial.quote + '</p>';
      html += '<span class="' + CSS_PREFIX + 'name">' + testimonial.name + '</span>';
      html += '</div>';
      ...
      var tiltPalette = settings.tiltColors || DEFAULTS.tiltColors;
      var tiltBg = tiltPalette[index % tiltPalette.length];
      var angle = settings.tiltAngle || DEFAULTS.tiltAngle;
      var rotation = (index % 2 === 0 ? -angle : angle);
      html += '<div class="' + CSS_PREFIX + 'tilted-card" style="background:' + tiltBg + '; transform: rotate(' + rotation + 'deg);">';
      html += QUOTE_ICON_SVG.replace('opacity="0.15"', 'opacity="0.25"').replace('width="32" height="32"', 'width="48" height="48"').replace('fill="currentColor"', 'fill="#ffffff"');
      html += '<p class="' + CSS_PREFIX + 'quote">' + testimonial.quote + '</p>';
      html += '<div class="' + CSS_PREFIX + 'tilted-divider"></div>';
      html += '<span class="' + CSS_PREFIX + 'name">' + testimonial.name + '</span>';
      if (testimonial.role) {
        html += '<span class="' + CSS_PREFIX + 'role">' + testimonial.role + '</span>';
      }
   ...
   *  NAVIGATION BUILDERS
   * ────────────────────────────────────────────────────────────── */
  function buildArrows(settings) {
    var prevContent, nextContent;

    if (settings.arrowStyle === 'text') {
      prevContent = '<span>' + settings.arrowPrevText + '</span>';
      nextContent = '<span>' + settings.arrowNextText + '</span>';
    } else if (settings.arrowPrevIcon) {
      prevContent = settings.arrowPrevIcon;
      nextContent = settings.arrowNextIcon || settings.arrowPrevIcon;
    } else {
      prevContent = DEFAULT_PREV_ICON;
      nextContent = DEFAULT_NEXT_ICON;
    }

    var container = document.createElement('div');
    container.className = CSS_PREFIX + 'arrows ' + CSS_PREFIX + 'nav-' + settings.arrowsPosition;

    var prev = document.createElement('button');
    prev.className = CSS_PREFIX + 'arrow-prev';
    prev.setAttribute('aria-label', 'Previous testimonial');
    prev.innerHTML = prevContent;

    var next = document.createElement('button');
    next.className = CSS_PREFIX + 'arrow-next';
    next.setAttribute('aria-label', 'Next testimonial');
    next.innerHTML = nextContent;

    container.appendChild(prev);
    container.appendChild(next);

    return { container: container, prevEl: prev, nextEl: next };
  }

  function buildDots(settings) {
    var container = document.createElement('div');
    container.className = CSS_PREFIX + 'dots ' + CSS_PREFIX + 'nav-' + settings.dotsPosition;
    return container;
  }

  function buildCounter(total, settings) {
    var container = document.createElement('div');
    container.className = CSS_PREFIX + 'counter ' + CSS_PREFIX + 'nav-' + settings.counterPosition;
    container.innerHTML =
      '<span class="' + CSS_PREFIX + 'counter-current">' + pad(1) + '</span>' +
      ' / ' +
      '<span class="' + CSS_PREFIX + 'counter-total">' + pad(total) + '</span>';
    return container;
  }
   ...
   *  INJECT STYLES
   * ────────────────────────────────────────────────────────────── */
  var mainStyleEl = null;

  function injectStyles() {
    if (mainStyleEl) return;
    mainStyleEl = document.createElement('style');
    mainStyleEl.id = CSS_PREFIX + 'styles';
    mainStyleEl.textContent = getCSS();
  ...
  function getCSS() {
    ...
    '  height: 100%;' +
    '  padding: var(--sdl-ts-card-padding, 32px);' +
    '  box-sizing: border-box;' +
    '  border: var(--sdl-ts-card-border, 1px solid #e5e5e5);' +
    '  border-radius: var(--sdl-ts-card-radius, 12px);' +
    '  background: var(--sdl-ts-card-bg, transparent);' +
    '}' +
    '.' + CSS_PREFIX + 'card .' + CSS_PREFIX + 'quote-icon {' +
    '  flex-shrink: 0;' +
    '  margin-bottom: 16px;' +
    '}' +
    '.' + CSS_PREFIX + 'card .' + CSS_PREFIX + 'stars {' +
    '  margin-bottom: 16px;' +
    '  color: #f5a623;' +
    '}' +
    '.' + CSS_PREFIX + 'card .' + CSS_PREFIX + 'star {' +
    '  margin-right: 2px;' +
    '}' +
    ...
    '}' +
    '.' + CSS_PREFIX + 'card .' + CSS_PREFIX + 'author {' +
    '  display: flex;' +
    '  align-items: center;' +
    '  gap: 12px;' +
    '  margin-top: auto;' +
    ...
    '  margin: 0 auto;' +
    '}' +
    '.' + CSS_PREFIX + 'spotlight .' + CSS_PREFIX + 'quote-icon {' +
    '  margin-bottom: 24px;' +
    '}' +
    ...
    '  font-style: italic;' +
    '}' +
    '.' + CSS_PREFIX + 'spotlight .' + CSS_PREFIX + 'author {' +
    '  display: flex;' +
    '  align-items: center;' +
    '  gap: 14px;' +
    '}' +
    ...
    '  margin: 0 0 24px 0 !important;' +
    '}' +
    '.' + CSS_PREFIX + 'minimal-card .' + CSS_PREFIX + 'name {' +
    '  margin-top: auto;' +
    '}' +
    ...
    '  border: var(--sdl-ts-card-border, 1px solid #e5e5e5);' +
    '}' +
    '.' + CSS_PREFIX + 'magazine-image {' +
    '  flex: 0 0 50%;' +
    '  position: relative;' +
    '  overflow: hidden;' +
    '}' +
    '.' + CSS_PREFIX + 'magazine-image img {' +
    '  width: 100% !important;' +
    ...
    '  font-weight: 400;' +
    '}' +
    '.' + CSS_PREFIX + 'colorful-card .' + CSS_PREFIX + 'author-info {' +
    '  margin-top: auto;' +
    '}' +
    ...
    '  border-radius: var(--sdl-ts-card-radius, 12px);' +
    '}' +
    '.' + CSS_PREFIX + 'review-card .' + CSS_PREFIX + 'stars {' +
    '  margin-bottom: 16px;' +
    '}' +
    ...
    '}' +
    '.' + CSS_PREFIX + 'tilted-card .' + CSS_PREFIX + 'quote-icon {' +
    '  margin-bottom: 16px;' +
    '}' +
    ...
    '  z-index: 10;' +
    '}' +
    '.' + CSS_PREFIX + 'arrow-prev,' +
    '.' + CSS_PREFIX + 'arrow-next {' +
    '  display: flex;' +
    '  align-items: center;' +
    '  justify-content: center;' +
    '  width: 44px;' +
    ...
    '  line-height: 1;' +
    '}' +
    '.' + CSS_PREFIX + 'arrow-prev:hover,' +
    '.' + CSS_PREFIX + 'arrow-next:hover {' +
    '  background: rgba(0,0,0,0.05);' +
    '}' +

    /* Arrow text style */
    '.' + CSS_PREFIX + 'arrows.sdl-ts-arrow-text .' + CSS_PREFIX + 'arrow-prev,' +
    '.' + CSS_PREFIX + 'arrows.sdl-ts-arrow-text .' + CSS_PREFIX + 'arrow-next {' +
    '  width: auto;' +
    '  border-radius: 22px;' +
    '  padding: 0 20px;' +
    '  font-weight: 500;' +
    ...
    '  gap: 6px;' +
    '}' +
    '.' + CSS_PREFIX + 'dots .swiper-pagination-bullet {' +
    '  width: 8px;' +
    '  height: 8px;' +
    '  border-radius: 50%;' +
    ...
    '  padding: 0;' +
    '}' +
    '.' + CSS_PREFIX + 'dots .swiper-pagination-bullet-active {' +
    '  opacity: 1;' +
    '}' +
    ...
    /* ── Nav positions ── */
    '.' + CSS_PREFIX + 'nav-bottom-center { bottom: 0; left: 50%; transform: translateX(-50%); }' +
    '.' + CSS_PREFIX + 'nav-bottom-left { bottom: 0; left: 0; }' +
    '.' + CSS_PREFIX + 'nav-bottom-right { bottom: 0; right: 0; }' +
    ...
    '.' + CSS_PREFIX + 'wrapper.sdl-ts-has-top-nav { padding-top: 60px; }' +
    ...
    '  .' + CSS_PREFIX + 'colorful-card,' +
    '  .' + CSS_PREFIX + 'review-card {' +
    '    padding: var(--sdl-ts-card-padding, 20px);' +
    '  }' +
    '}' +

    /* ── Hide original summary block ── */
    '.' + CSS_PREFIX + 'source-hidden {' +
   ...
   *  BUILD SLIDER
   * ────────────────────────────────────────────────────────────── */
  function buildSlider(sectionEl, testimonials, settings) {
    if (!testimonials.length) return null;

    var summaryBlock = sectionEl.querySelector('.sqs-block-summary-v2');
    var siteStyles = readSiteStyles(sectionEl);
    ...
    if (settings.cardBorder) {
      wrapper.style.setProperty('--sdl-ts-card-border', settings.cardBorderWidth + 'px solid ' + settings.cardBorderColor);
    } else {
      wrapper.style.setProperty('--sdl-ts-card-border', 'none');
    }
    wrapper.style.setProperty('--sdl-ts-card-radius', settings.cardBorderRadius + 'px');
    ...
    if (siteStyles.fontFamily) {
      wrapper.style.fontFamily = siteStyles.fontFamily;
    }
    if (siteStyles.bodyColor) {
      wrapper.style.color = siteStyles.bodyColor;
    }
    ...
    function checkNavPosition(pos) {
      if (!pos) return;
      if (pos.indexOf('bottom') === 0) hasBottomNav = true;
      if (pos.indexOf('top') === 0) hasTopNav = true;
    }

    // Build navigation
    var arrowData = null;
    if (settings.arrows) {
      arrowData = buildArrows(settings);
      if (settings.arrowStyle === 'text') {
        arrowData.container.classList.add(CSS_PREFIX + 'arrow-text');
      }
      wrapper.appendChild(arrowData.container);
      checkNavPosition(settings.arrowsPosition);
    }

    var dotsEl = null;
    if (settings.dots) {
      dotsEl = buildDots(settings);
      wrapper.appendChild(dotsEl);
      checkNavPosition(settings.dotsPosition);
    }

    var counterEl = null;
    if (settings.counter) {
      counterEl = buildCounter(testimonials.length, settings);
      wrapper.appendChild(counterEl);
      checkNavPosition(settings.counterPosition);
    }

    if (hasBottomNav) wrapper.classList.add(CSS_PREFIX + 'has-bottom-nav');
    if (hasTopNav) wrapper.classList.add(CSS_PREFIX + 'has-top-nav');

    // Insert into DOM — after summary block, then hide original
    if (summaryBlock) {
      summaryBlock.parentNode.insertBefore(wrapper, summaryBlock.nextSibling);
      summaryBlock.classList.add(CSS_PREFIX + 'source-hidden');
    } else {
      sectionEl.querySelector('.sqs-section .sqs-row, .page-section .content-wrapper, .content')
        ? sectionEl.querySelector('.sqs-section .sqs-row, .page-section .content-wrapper, .content').appendChild(wrapper)
  ...
  function initSwiper(built, settings, totalSlides) {
    var config = {
      ...
      breakpoints: {
        0: {
          ...
          spaceBetween: Math.min(settings.spaceBetween, 16),
        },
        768: {
          slidesPerView: settings.tabletSlides,
          spaceBetween: settings.spaceBetween,
        },
        1024: {
          slidesPerView: settings.slidesPerView,
    ...
    if (built.arrowData) {
      config.navigation = {
        prevEl: built.arrowData.prevEl,
        nextEl: built.arrowData.nextEl,
      };
    }
    ...
    // Counter update
    if (built.counterEl) {
      var currentEl = built.counterEl.querySelector('.' + CSS_PREFIX + 'counter-current');
  ...
  function init() {
    ...
    var sections = findSections().filter(function (s) { return !s.dataset.sdlTsState; });
    if (!sections.length) {
      removeFOUCStyles();
      return;
    }
    ...
    sections.forEach(function (sectionEl) {
      sectionEl.dataset.sdlTsState = 'loading';

      try {
        var settings = getSettings(sectionEl);
        var testimonials = extractTestimonials(sectionEl, settings);

        if (!testimonials.length) {
          console.warn('[SDL Testimonial Slider] No summary items found in #' + sectionEl.id);
          sectionEl.dataset.sdlTsState = 'error';
          return;
        }

        var built = buildSlider(sectionEl, testimonials, settings);
        if (!built) {
          sectionEl.dataset.sdlTsState = 'error';
          return;
        }

        var swiper = initSwiper(built, settings, testimonials.length);

        built.wrapper._sdlTs = {
          swiper: swiper,
          sectionEl: sectionEl,
          settings: settings,
        ...
        sectionEl.dataset.sdlTsState = 'ready';

        emitEvent(NAMESPACE + ':ready', { swiper: swiper, settings: settings }, sectionEl);
      } catch (err) {
        console.error('[SDL Testimonial Slider] Init error in #' + sectionEl.id + ':', err);
        sectionEl.dataset.sdlTsState = 'error';
      }
    });
  ...
  /* ──────────────────────────────────────────────────────────────────
   *  DECONSTRUCT (edit mode / re-init)
   * ────────────────────────────────────────────────────────────── */
  function deconstruct(sectionId) {
    var wrappers = document.querySelectorAll('.' + CSS_PREFIX + 'wrapper');
    wrappers.forEach(function (wrapper) {
      var data = wrapper._sdlTs;
      if (!data) return;

      // If sectionId is specified, only deconstruct that one
      if (sectionId && data.sectionEl && data.sectionEl.id !== sectionId) return;

      if (data.swiper) data.swiper.destroy(true, true);

      // Show original summary block
      if (data.summaryBlock) {
        data.summaryBlock.classList.remove(CSS_PREFIX + 'source-hidden');
      }

      // Clear section state
      if (data.sectionEl) {
        delete data.sectionEl.dataset.sdlTsState;
      }
   ...
   *  EDIT MODE OBSERVER
   * ────────────────────────────────────────────────────────────── */
  var editModeObserverSet = false;

  function setupEditModeObserver() {
    if (editModeObserverSet) return;
   ...
   *  PUBLIC API
   * ────────────────────────────────────────────────────────────── */
  window.sdlTestimonialSlider = {
    init: init,
    destroy: function (sectionId) { deconstruct(sectionId); },
  };
